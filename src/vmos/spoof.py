...
def set_build_props(
    ...
    *,
    ...
    values are also written into the **valid** ``vmos_spoof`` Magisk module
    (``module.prop`` + ``system.prop``, the same reboot-durable mechanism
    :func:`set_identity_props` uses) — ``system.prop`` is merged so these
    ``persist.vmos.spoof.build.*`` keys and the identity inputs never clobber each
    other regardless of call order. Returns the ``prop -> value`` map written.
    """
    shell = PadRootShell(client, pad_code)
    values = {
    ...
    _run_batched(shell, resetprop_commands(to_set))
    if persist_module:
        # Persist as a VALID Magisk module so the Build.* props survive reboot,
        # re-applied every boot at post-fs-data via system.prop (NOT the volatile
        # runtime `resetprop -n`, which is lost on reboot). system.prop is a
        # read-merge-write via _write_module_system_prop, so the identity inputs
        # (persist.vmos.spoof.*) written by set_identity_props and these Build.*
        # keys (persist.vmos.spoof.build.*) coexist in the one module regardless of
        # call order — neither layer clobbers the other. All writes go through
        # _write_file / _write_module_system_prop, respecting ASYNC_CMD_MAX_BYTES.
        _write_file(shell, f"{_MODULE_DIR}/module.prop", _module_prop())
        _write_module_system_prop(shell, to_set)
        # Ensure the module is enabled (no disable/remove flags).
    ...
    **Hard guard (design §B):** refuses ``com.google.android.gms`` and
    ...
    if target_pkg in GMS_DENYLIST:
        raise ValueError(
            ...
            "only, never a GMS_DENYLIST package (com.google.android.gms / "
