...
Install/uninstall, start/stop/restart apps, list installed apps, keep-alive and hidden-app lists.
...
| `props` | `props` | Object | no | System properties (key-value) |
...
| `auto_install` | `autoInstall` | Integer | no | Auto install: 1-yes, 0-no (default no). Only for APK |
| `file_unique_id` | `fileUniqueId` | String | no | File unique ID |
| `customize_file_path` | `customizeFilePath` | String | no | Custom path (start with /, e.g. "/DCIM/", "/Documents/" etc.) |
| `file_name` | `fileName` | String | no | File name |
| `package_name` | `packageName` | String | no | Package name |
| `url` | `url` | String | no | File URL |
| `md5` | `md5` | String | no | File MD5 |
| `is_authorization` | `isAuthorization` | Boolean | no | Grant permissions (default all) |
| `icon_path` | `iconPath` | String | no | Icon for install |
...
| Python argument | API name | Type | Required | Description |
|---|---|---|---|---|
| `list` | `list` | Object[] | yes | Upload item list, up to 100 items per call |

**Nested fields of `list`:**
...
| `app_name` | `appName` | String | no | App name |
...
| Python argument | API name | Type | Required | Description |
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | no | Instance codes |
| `apply_all_instances` | `applyAllInstances` | Boolean | yes | Apply to all instances mode |
| `app_infos` | `appInfos` | Object[] | no |  |
...
| Python argument | API name | Type | Required | Description |
|---|---|---|---|---|
| `name` | `name` | String | yes | ROM name |
| `update_log` | `updateLog` | String | yes | Update log |
| `android_version` | `androidVersion` | String | yes | Android version |
| `version` | `version` | String | yes | Version |
| `download_url` | `downloadUrl` | String | yes | Download URL |
| `package_size` | `packageSize` | String | yes | Size (bytes) |
...
| Python argument | API name | Type | Required | Description |
|---|---|---|---|---|
| `apps` | `apps` | Object[] | yes | Application list |

**Nested fields of `apps`:**
...
| Python argument | API name | Type | Required | Description |
|---|---|---|---|---|
| `pkg_name` | `pkgName` | String | yes | Package Name |
| `pad_codes` | `padCodes` | String[] | yes |  |
...
|---|---|---|---|---|
| `pkg_name` | `pkgName` | String | yes | Package Name |
| `pad_codes` | `padCodes` | String[] | yes | Instance IDs |
