# `client.instance` — Quản lý phiên bản (Instance)
...
| `wifi_json_list` | `wifiJsonList` | String[] | có | WIFI properties list |

**Các trường con của `wifiJsonList`:**
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `last_id` | `lastId` | Long | không | Last query returned lastId; null for first query |
| `rows` | `rows` | Integer | không | Records per page, max 1000 per page |
| `pad_codes` | `padCodes` | String[] | không |  |
| `pad_ips` | `padIps` | String[] | không |  |
| `online` | `online` | Integer | không | Instance online status: 0-offline, 1-online |
| `pad_status` | `padStatus` | Integer | không | Instance running status: 14-abnormal, others-normal |
| `compute_occupied` | `computeOccupied` | Boolean | không | Filter compute occupation: true-occupied, false-not occupied |
| `net_storage_res_flag` | `netStorageResFlag` | Integer | không | Net storage flag: 1-net storage instance, 0-local instance |
| `brand` | `brand` | String | không | Instance brand (exact match) |
| `brand_model` | `brandModel` | String | không | Brand model (exact match) |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có | Instance count not exceeding 200 |
...
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có |  |
| `modem_persist_properties_list` | `modemPersistPropertiesList` | Object[] | không | Modem persistent properties list |
| `modem_properties_list` | `modemPropertiesList` | Object[] | không | Modem non-persistent properties list |
| `system_persist_properties_list` | `systemPersistPropertiesList` | Object[] | không | System persistent properties list |
| `system_properties_list` | `systemPropertiesList` | Object[] | không | System non-persistent properties list |
| `setting_properties_list` | `settingPropertiesList` | Object[] | không | Setting properties list |
| `oaid_properties_list` | `oaidPropertiesList` | Object[] | không | OAID properties list |
...
**Các trường con của `modemPersistPropertiesList`:**
...
**Các trường con của `modemPropertiesList`:**
...
**Các trường con của `systemPersistPropertiesList`:**
...
**Các trường con của `systemPropertiesList`:**
...
**Các trường con của `settingPropertiesList`:**
...
**Các trường con của `oaidPropertiesList`:**
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `pad_code` | `padCode` | String | không | Instance ID |
| `restart` | `restart` | Boolean | không | Auto restart after setting (default false) |
| `props` | `props` | Object | có | System properties (key-value) |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `task_id` | `taskId` | String | có | Task ID returned by updateSIM |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `host` | `host` | String | có | Proxy info (IP or host) |
| `port` | `port` | Integer | có | Proxy port (numeric) |
| `account` | `account` | String | có | Proxy username |
| `password` | `password` | String | có | Proxy password |
| `type` | `type` | String | có | Proxy protocol: Socks5, http, https |
| `country` | `country` | String | không | Country - required when forcing specification |
| `ip` | `ip` | String | không | IP - required when forcing |
| `loc` | `loc` | String | không | Latitude, longitude - required when forcing |
| `city` | `city` | String | không | City - required when forcing |
| `region` | `region` | String | không | Region - required when forcing |
| `timezone` | `timezone` | String | không | Timezone - required when forcing |
...
| `pad_codes` | `padCodes` | String[] | có |  |
| `host` | `host` | String | có | Proxy info (IP or host) |
| `port` | `port` | Integer | có | Proxy port |
| `account` | `account` | String | có | Proxy username |
| `password` | `password` | String | có | Proxy password |
| `type` | `type` | String | có | Proxy protocol: socks5, http, https |
| `mode` | `mode` | String | có | Proxy mode: vpn / proxy |
| `bypass_package_list` | `bypassPackageList` | Array | không | Bypass packages |
| `bypass_ip_list` | `bypassIpList` | Array | không | Bypass IPs |
| `bypass_domain_list` | `bypassDomainList` | Array | không | Bypass domains |
| `follow_language` | `followLanguage` | Boolean | không | Whether device language follows the proxy IP |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `pad_code_list` | `padCodeList` | String[] | có |  |

**Các trường con của `padCodeList`:**
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `time_zone` | `timeZone` | String | có | UTC standard timezone |
| `pad_codes` | `padCodes` | Array | có | Instance list |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `language` | `language` | String | có | Language |
| `country` | `country` | String | không | Country |
| `pad_codes` | `padCodes` | Array | có | Instance list |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `longitude` | `longitude` | Float | có | Longitude |
| `latitude` | `latitude` | Float | có | Latitude |
| `altitude` | `altitude` | Float | không | Altitude (requires latest image) |
| `speed` | `speed` | Float | không | Speed m/s (images after 20251024) |
| `bearing` | `bearing` | Float | không | Bearing ° (images after 20251024) |
| `horizontal_accuracy_meters` | `horizontalAccuracyMeters` | Float | không | Horizontal accuracy (images after 20251024) |
| `pad_codes` | `padCodes` | Array | có | Instance list |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `pad_codes` | `padCodes` | Array | có | Instance ID list |
| `country_code` | `countryCode` | String | không | Country code (see: https://chahuo.com/country-code-lookup.html) |
| `real_phone_template_id` | `realPhoneTemplateId` | Long | không | Template ID (refer to [Paginated Get Real Device Templates]) |
| `android_prop` | `androidProp` | Object | không | Refer to [Android Modification Properties List] |
| `replacement_real_adi_flag` | `replacementRealAdiFlag` | Boolean | không | Whether random ADI template for real device (false-no, true-yes) |
| `exclude_real_phone_template_ids` | `excludeRealPhoneTemplateIds` | Long[] | không | Exclude template IDs when random |
| `certificate` | `certificate` | String | không | Phone root certificate |
| `wipe_data` | `wipeData` | Boolean | không | Clear user data (default true, CBS2.4.4+ support) |
| `wipe_specific_data` | `wipeSpecificData` | String[] | không | Effective when wipeData false; specify data to clear |
| `keep_specific_data` | `keepSpecificData` | String[] | không | Effective when wipeData is false; specifies which data should be preserved (mutually exclusive with wipeSpecificData) |
| `enable_cpu_core_config` | `enableCpuCoreConfig` | Boolean | không | Enable CPU core config (based on Android cpuset for performance stability) |
| `webview_id` | `webviewId` | Long | không | WebView version ID, see [Query Available WebView Versions](https://cloud.vmoscloud.com/vmoscloud/doc/en/server/OpenAPI.html#query-available-webview-versions) |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có | Instance codes (does not support `ACN` prefix) |
| `set_proxy_flag` | `setProxyFlag` | Boolean | có | Set to `true` to write SIM based on deployment location |
| `wipe_data` | `wipeData` | Boolean | không | Whether to clear data; default `true` |
| `keep_lang_timezone` | `keepLangTimezone` | Boolean | không | Whether to keep old language/timezone; default `false` |
| `android_prop` | `androidProp` | Object | không | Custom Android system properties as key-value pairs. Examples: `persist.sys.locale`, `persist.sys.timezone`, etc. Higher priority than auto-generated properties |
| `webview_id` | `webviewId` | Long | không | WebView version ID, see [Query Available WebView Versions](https://cloud.vmoscloud.com/vmoscloud/doc/en/server/OpenAPI.html#query-available-webview-versions) |
...
|---|---|---|---|---|
| `pad_codes` | `padCodes` | Array | có | Instance list |
| `format` | `format` | String | không | Image format: png, jpg (default png; png no compression) |
| `height` | `height` | String | không | Scaled height (pixels; default original) |
| `width` | `width` | String | không | Scaled width (pixels; default original) |
| `quality` | `quality` | Integer | không | Image quality (0-100; default 50%; below 60 blurry) |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `account` | `account` | String | không | Username |
| `password` | `password` | String | không | Password |
| `ip` | `ip` | String | không | IP |
| `port` | `port` | Integer | không | Port |
| `enable` | `enable` | Boolean | có | Enable |
| `pad_codes` | `padCodes` | Array | có | Instance list |
| `proxy_type` | `proxyType` | String | không | Supported: proxy, vpn |
| `proxy_name` | `proxyName` | String | không | Supported: socks5, http-relay (includes http/https) |
| `model` | `model` | String | không | List mode: bypass(default)/limit |
| `bypass_package_list` | `bypassPackageList` | Array | không | Packages bypassing proxy (model=bypass) |
| `bypass_ip_list` | `bypassIpList` | Array | không | IPs bypassing proxy (model=bypass) |
| `bypass_domain_list` | `bypassDomainList` | Array | không | Domains bypassing proxy (model=bypass) |
| `limit_package_list` | `limitPackageList` | Array | không | Only these packages use proxy (model=limit) |
| `limit_ip_list` | `limitIpList` | Array | không | Only these IPs use proxy (model=limit) |
| `limit_domain_list` | `limitDomainList` | Array | không | Only these domains use proxy (model=limit) |
| `s_uo_t` | `sUoT` | Boolean | không | Enable UDP (default false) |
...
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có |  |
| `script_content` | `scriptContent` | String | có | ADB commands (multiple separated by “;” ) |
...
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có |  |
| `global_root` | `globalRoot` | Boolean | không | Global root (default no) |
| `package_name` | `packageName` | String | không | Package name (required for non-global; multiple comma separated) |
| `root_status` | `rootStatus` | Integer | có | Root status: 0-off, 1-on |
...
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có |  |
| `rotation` | `rotation` | Integer | có | Screenshot orientation: 0-default, 1-rotate to portrait |
| `broadcast` | `broadcast` | Boolean | không | Broadcast event (default false) |
| `definition` | `definition` | Integer | không | Clarity 0-100 |
| `resolution_height` | `resolutionHeight` | Integer | không | Height >1 |
| `resolution_width` | `resolutionWidth` | Integer | không | Width >1 |
...
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có |  |
| `image_id` | `imageId` | String | có | Image ID |
| `wipe_data` | `wipeData` | Boolean | có | Clear data partition: true-yes, false-no |
| `enable_cpu_core_config` | `enableCpuCoreConfig` | Boolean | không | Enable CPU core config |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có | Array of instance codes (maximum 200) |
| `app_infos` | `appInfos` | Object[] | có | Array of hidden app list objects; pass empty array [] to clear (0–200 items) |
...
| `pad_codes` | `padCodes` | String[] | có |  |
| `image_id` | `imageId` | String | có | Image ID |
| `wipe_data` | `wipeData` | Boolean | có | Clear data: true-yes, false-no |
| `real_phone_template_id` | `realPhoneTemplateId` | Integer | không | Real device template ID (required for real) |
| `upgrade_image_convert_type` | `upgradeImageConvertType` | String | có | Convert type: virtual / real |
| `screen_layout_id` | `screenLayoutId` | Integer | không | Screen layout ID (required for virtual) |
| `certificate` | `certificate` | String | không | Custom root certificate |
| `device_android_props` | `deviceAndroidProps` | Object | không | Android props (CBS <2.4.4 not support) |
| `enable_cpu_core_config` | `enableCpuCoreConfig` | Boolean | không | Enable CPU core config |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `page` | `page` | Integer | không | Page number, default 1 |
| `rows` | `rows` | Integer | không | Number of items per page, default 10, range 1-100 |
...
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có |  |
| `up_bandwidth` | `upBandwidth` | float | có | Upload bandwidth Mbps (0: unlimited; -1: block internet) |
| `down_bandwidth` | `downBandwidth` | float | có | Download bandwidth Mbps (0: unlimited; -1: block internet) |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có | Instance list (1-200 instances) |
| `open_status` | `openStatus` | Integer | có | ADB status (1: enable; 0 or omit: disable) |
...
|---|---|---|---|---|
| `pad_code` | `padCode` | String | có | Instance code |
| `enable` | `enable` | Boolean | có | ADB status: true-enable, false-disable |
| `expire_minutes` | `expireMinutes` | Integer | không | ADB validity period in minutes (1–7 days, i.e. 1440–10080), default 1440 |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có | Instance code list (max 10) |
| `enable` | `enable` | Boolean | có | Enable ADB: true-enable and return info, false-disable |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có | List of instance codes to transfer |
| `make_over_mobile_phone` | `makeOverMobilePhone` | String | có | Recipient account email |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `task_ids` | `taskIds` | Integer[] | có | Array length 1-100 |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `page` | `page` | Integer | có | Page number |
| `rows` | `rows` | Integer | có | Records per page |
| `pad_type` | `padType` | String | không | Instance type (virtual: virtual; real: real) |
| `pad_codes` | `padCodes` | String[] | không |  |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có | Instances to edit call logs |
| `call_records` | `callRecords` | Object[] | có | Call logs |

**Các trường con của `callRecords`:**
...
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có | Instance codes |
| `text` | `text` | String | có | Input text |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có | Instance list (1-100) |
| `sender_number` | `senderNumber` | String | có | Sender number (no mainland; max 16 chars, digits/letters/space/+-) |
| `sms_content` | `smsContent` | String | có | SMS content (max 127 chars) |
...
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có |  |
| `reset_gms_type` | `resetGmsType` | String | có | Reset type: GAID |
| `opr_by` | `oprBy` | String | không | Operator |
| `task_source` | `taskSource` | String | có | Task source: OPEN_PLATFORM |
...
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có | Instance codes |
| `url` | `url` | String | không | Audio download URL (one of url/fileUniqueId) |
| `file_unique_id` | `fileUniqueId` | String | không | File unique ID (one of url/fileUniqueId) |
| `enable` | `enable` | Boolean | có | Inject switch |
...
| Tham số Python | Tên trong API | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có | Instances (1-100) |
| `inject_switch` | `injectSwitch` | Boolean | không | Enable injection (true: on; false: off; default false) |
| `inject_loop` | `injectLoop` | Boolean | không | Loop playback (default false) |
| `inject_url` | `injectUrl` | String | không | Single video URL (http/https/rtmp:// or local; one with injectUrls) |
| `inject_urls` | `injectUrls` | String[] | không | Video URL list (max 5; one with injectUrl) |
...
|---|---|---|---|---|
| `pad_codes` | `padCodes` | String[] | có | Instances (1-100) |
| `inject_switch` | `injectSwitch` | Boolean | không | Enable (true: on; false: off; default false) |
| `inject_loop` | `injectLoop` | Boolean | không | Loop (default false) |
| `inject_url` | `injectUrl` | String | có | Image URL (http/https/rtmp://) |
